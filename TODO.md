...
**NEXT ACTION (2026-07-19):** Main session: review the 6 queued competitor-research commits (`615189b` docs → `ee00fbf` teardown offer → `7c4f5bd` guarantee badge → `533ca72` /for/* trade pages → `245ef70` two-door chooser → docs/receipts), confirm the refund-badge policy with Mike (PENDING_MANUAL), then make the session's one push. Post-deploy: 375px visual smoke on `/for/outdoor-living` + homepage two-door, and sweep the new PENDING_MANUAL § competitor-research gates.
...
- [x] **[high/S] (trust)** Add a one-line guarantee badge to the $500 and $1k-2k pricing cards: 'Full refund any time before launch.' (Avoid date-based penalties like '14 days or $250 back' until build-time data from a few more projects supports it.) One small Astro badge component + copy. **DONE 2026-07-19 `7c4f5bd`** — badge on the two build-lane cards only, pin test locks copy + tier placement + no-date-penalty rule; Mike's honor-confirmation → PENDING_MANUAL before push.
...
- [ ] **[high/M] (flow)** Make the $500 Basic tier directly buyable: Stripe Payment Link (full or deposit) on the card, build-week slot picker, intake form shown after payment. New /start page in Astro + Payment Link + n8n webhook into the existing lead OS. Keep quote flow for Business/Premium. **BLOCKED 2026-07-19** — needs Stripe dashboard + full-vs-deposit policy call → PENDING_MANUAL; /start build is agent-actionable once the Payment Link URL exists.
...
- [x] Add a guarantee badge to the $500 and $1k-2k cards ('Full refund any time before launch') — the site currently has zero risk-reversal vs Footbridge's 90-day and Designjoy's week-one guarantees; one Astro component + copy, ~2 hours. **DONE 2026-07-19 `7c4f5bd`** (same commit as the upgrade item above).
