...
## D-CS-011 · 2026-08-03 · Referral program ships in capture-only mode; the payout number is owner-gated in code

- **Rule:** The referral feature ships complete except for the cash figure. `functions/_lib/referral.ts` is the single source of truth for both the amount and the copy that states it; `bountyUsd: null` renders a variant that makes no payment promise, and `tests/build/referral-program.test.ts` fails if any payout figure reaches a public surface while it is null.
