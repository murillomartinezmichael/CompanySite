...
// ffprobe verified this exact file has one H.264 video stream and no audio
// (2026-09-09). A changed asset must be reviewed again, never silently exempted.
const silentReelVerified = createHash('sha256')
...
try {
  for (const viewport of VIEWPORTS) {
    for (const route of ROUTES) {
      ...
      await page.keyboard.press('Tab');
      const skipWorks = await page.evaluate(expected => document.activeElement.getAttribute('data-cta') === expected, firstCta);
      ...
      const silentVideoReview = silentReelVerified && await page.evaluate(() => {
        const video = document.querySelector('video[aria-describedby="case-study-aries-details"]');
        const source = video?.querySelector('source');
        const path = source?.getAttribute('src') || source?.getAttribute('data-src');
        return path === '/videos/aries-scroll-v2.mp4' && Boolean(video.getAttribute('aria-label'))
          && document.getElementById('case-study-aries-details')?.textContent.trim().length > 100;
      });
